...
public class DatabaseBackend extends SQLiteOpenHelper {
    ...
    public void deleteMessagesInUuid(String uuid) {
        long start = SystemClock.elapsedRealtime();
        final SQLiteDatabase db = this.getWritableDatabase();
        db.beginTransaction();
        String[] args = {uuid};
        Cursor cusrsor=  db.query(Message.TABLENAME,new String[]{Message.CONVERSATION},"uuid =?", args,null,null,null);
        String conversionId[]=new String[1];
        if(cusrsor.moveToNext()){
            conversionId[0]=cusrsor.getString(0);
        }
        System.out.println("converstion id"+conversionId[0]);
        int delete2=db.delete(Conversation.TABLENAME,Conversation.UUID+"=?",conversionId);
        System.out.println("delete2=>"+delete2);
        int delte = db.delete("messages_index", "uuid =?", args);
